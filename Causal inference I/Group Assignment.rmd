...
data_df = data_df[!apply(data_df, MARGIN = 1, anyNA), ]
```
```{r}
outcome_r = glm(DEATH ~ BPMEDS + EDUC + SYSBP + DIABP + BPMEDS:EDUC + BPMEDS:SYSBP + BPMEDS:DIABP + SEX + AGE + CURSMOKE + BMI + HEARTRTE, data = data_df, family = 'binomial')
summary(outcome_r)
```
```{r}
n = nrow(data_df)
